package me.emmy.basic.menus.settings;
...
public class SettingsMenu extends Menu {
	...
	public SettingsMenu() {
		this.refillGlassButton = new RefillGlassButton(
				...
				Basic.getInstance().getConfig().getInt("settings-menu.refill-glass.data", 15)
		);
	}

	@Override
	public String getTitle(Player player) {
		return CC.translate(Basic.getInstance().getConfig().getString("settings-menu.title"));
	}
	...
	public Map<Integer, Button> getButtons(Player player) {
		...
		ConfigurationSection serversSection = Basic.getInstance().getConfig().getConfigurationSection("settings-menu.items");
		...
		if (serversSection != null) {
			for (String serverKey : serversSection.getKeys(false)) {
				...
				if (serverSection != null) {
					...
					buttons.put(slot, new SettingsButton(material, (short) data, name, lore, serverSection.getString("command")));
				}
			}
		}

		ConfigurationSection refillGlassSection = Basic.getInstance().getConfig().getConfigurationSection("settings-menu.refill-glass");
	...
	public int getSize() {
		...
		if (newsConfig != null) {
			return newsConfig.getInt("settings-menu.size", 9 * 3);
		}
